...
var permissionsSchema = new Schema({
    ...
    description : { type: String , required: false, unique: false },
    parent      : { type: Number , required: false, unique: false }
});
...
permissionsSchema.methods.update = function(data, callback) {
    if (err) return callback(err);
...
permissionsSchema.statics.create = function(options, callback) {
...
permissionsCategorySchema.statics.create = function(options, callback) {
    ...
    permissionsCategory.save(function(err) {
        if (err) return callback(err);
        else return callback();
    });
};
...
exports.permissionsCategoryModel = permissionsCategoryModel = mongoose.model('PermissionsCategory', permissionsCategorySchema);
