...
router.get('/:hostname/package/:pkg/',     renderWithPkg('panel/server/packages/view'));
router.get('/:hostname/services/running',  r.findServerAndRender('panel/server/services/running'));
router.get('/:hostname/services',          r.findServerAndRender('panel/server/services/list'));
router.get('/:hostname/service/:service/', renderWithService('panel/server/services/view'));
router.get('/:hostname/processes',         reserved);
router.get('/:hostname/process/:process/', reserved);

function renderWithPkg(path) {
...
function renderWithService(path) {
    return function(req, res) {
        r.findServer({ hostname: req.params.hostname }, function(result) {
            res.render(path, {
