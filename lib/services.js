...
function serializeServiceInfo(service) {
    ...
    while ((m = re.exec(service)) !== null) {
        ...
        if (!!m[1] && !!m[2]) {
            var key = serviceInfo[m[1].replace('-', '').toLowerCase()];

            if (!!key) {
                if (typeof key == 'object') serviceInfo[key].push(m[2]);
                else {
            ...
            serviceInfo[m[1].replace('-', '').toLowerCase()] = m[2];
        }
        if (!!m[3]) serviceInfo.name = m[3];
        if (!!m[4]) serviceInfo.description = m[4];
        if (!!m[5] && !!m[6] && !!m[7] && !!m[8]) {
