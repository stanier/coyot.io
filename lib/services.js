    ...
    io.on('connection', function(socket) {
        ...
        socket.on('start service', function(service) {
            ...
            childProcess.on('exit', function(code, signal) {
                if (code === 0) socket.emit('start service response', service, 'success');
                if (code === 1) socket.emit('start service response', service, 'failure');
            });
        ...
        socket.on('stop service', function(service) {
            ...
            childProcess.on('exit', function(code, signal) {
                if (code === 0) socket.emit('stop service response', service, 'success');
                if (code === 1) socket.emit('stop service response', service, 'failure');
            });
            childProcess.stdout.pipe(through(function(output, encoding, next) {
        ...
        socket.on('restart service', function(service) {
            ...
            childProcess.on('exit', function(code, signal) {
                if (code === 0) socket.emit('restart service response', service, 'success');
                if (code === 1) socket.emit('restart service response', service, 'failure');
            });
            childProcess.stdout.pipe(through(function(output, encoding, next) {
                console.log(output);
                return next();
        ...
        socket.on('get service status', function(service) {
            try {
            ...
            childProcess.stdout.pipe(through(function(output, encoding, next) {
                var status = /\bactive/g.test(output);
                socket.emit('service status response', service, status);
        ...
        socket.on('get status all', function() {
            ...
            childProcess.stdout.pipe(through(function(output, encoding, next) {
                ...
                socket.emit('service status all response', result[2], (result[1] == '+'));
                return next();
            }));
            childProcess.stderr.pipe(socketErr);
        });
    ...
    router.get('', function(req, res) {
        ...
        .then(function(list) {
            ...
            if (!!req.query.max) {
                ...
                response.services = list.slice(offset, offset + parseInt(req.query.max));
            } else {
        ...
        return util.forDistro({
            'ubuntu': function() {
                return exec('service ' + req.params.service + ' status', function(err, stdout, stderr) {
                    ...
                    if (err && err.code !== 3) return res.send(err);
...
function serializeServiceInfo(service) {
    ...
    while ((match = re.exec(service)) !== null) {
        ...
        serviceInfo.isOnline = /\bactive/g.test(serviceInfo.active);
    }
